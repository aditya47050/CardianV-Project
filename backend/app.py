...
# Initialize Pretrained Model Evaluator
...
async def predict_heart_sound(file: UploadFile = File(...)):
    ...
    if not model_evaluator.is_loaded:
        return {
            ...
            "error": f"Pretrained weights file (heartbeat-anomaly-detector-model.pt) not found in {model_evaluator.checkpoint_path}. Download from https://huggingface.co/ai-mitra/heartbeat-anomaly-detector and place in backend/checkpoints/."
        }
